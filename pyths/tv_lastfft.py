...
"""tv_fftone2all.py: Computes the FFT and Features last record"""
...
#    mags -= max(mags)
    ...
    print "length of mags",len(mags)
    f = open('/tmp/avgxyzspec.csv', 'w')
...
def main():
    # input data
    if (len(sys.argv) < 2):
        print "Usage: python tv_fftone2all.y hostname" 
        print "     : or ./tv_fftone2all.py hostname"
        print "     : "
        exit(0)
    else:
        host = sys.argv[1]
    ...
    sttmnt = """SELECT subjectid FROM TremCamTbl WHERE email=%s order by subjectid desc limit 1"""
    try:
