#!/usr/bin/python
# -*- coding: utf-8 -*-
"""tc_ft_avg_once.py: Computes the average FFT for a number of records
    trying to get """
...
#    statement = """UPDATE TremCamTbl SET avgfftspectrum= COALESCE(avgfftspectrum, %s), samplingHz= COALESCE(samplingHz,%s), datentime=%s WHERE subjectid=%s"""
    statement = """UPDATE TremCamTbl SET datentime=%s,avgfftspectrum=IF(avgfftspectrum IS NULL,avgfftspectrum, %s) WHERE subjectid=%s"""
    try:
...
def get_spectral_magnitude(y_data,time_data):
    ...
    if (len(y_data) == len(time_data) ):
        fs = len(timeV)/(np.max(timeV)*1e-3)
    else: 
        print 'input arrays do no match'
        print 'y_data:%d, time_data:%d' % (len(y_data),len(time_data))
        return [0,0]
    #print 'fs:',fs
...
def main():
    # input data
    if (len(sys.argv) < 2):
        print "Usage: python tc_ft_avg_once.py hostname user_email" #`"sql query`""
        print "     : or ./tc_ft_avg_once.py hostname user_email"
        exit(0)
    else:
        host = sys.argv[1]
        user = sys.argv[2]
...
#       acMtrx[range(0,len(ta)),i] 
        ...
        print 'timeVector:%d, avgdAxis:%d' % (len(timeVector), len(avgdAxis))
        mag_spectrum_vector,fsForRec = get_spectral_magnitude(avgdAxis,timeVector)
