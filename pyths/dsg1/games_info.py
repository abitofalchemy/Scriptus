...
import csv
from plotHumanPahts import save_fig
...
if __name__ == "__main__":
    ...
    if do == 0:
    ## compute stats on sp, cp, hp
        df = pd.DataFrame.from_csv("output.csv",header=False,sep=",")
    #print df.head()
    #sp_df = pd.DataFrame.from_csv("ssspGamesDatFiles/4153562_games_sp.dat",sep=",")
    #print sp_df.head()

    #sp_df.boxplot(column='sp', grid=False)
        fig = plt.figure(1, figsize=(9,6))
        ax  = fig.add_subplot(111)

#    plt.subplots_adjust(left=0.75, right=0.95, top
        bp = plt.boxplot(df.sp.values.tolist())
        ax.set_title('Test')
