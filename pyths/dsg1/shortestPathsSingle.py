    ...
    with open(sssp_file, 'r') as f:
        with contextlib.closing(mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ)) as m:
            for endPageId in dest_page_id_lst:
                pattern = re.compile(r'^%s\t[0-9]{1,2}' % endPageId,
                     re.DOTALL | re.IGNORECASE | re.MULTILINE)
                mp = pattern.search(m)
                if mp is not None:
                    #spScore.append(match[1].replace('\n', ' '))
                    #print endPageId, match[1].replace('\n', ' ')
                    spScore.append(mp.group().split('\t')[1])
                    #print endPageId, mp.group().split('\t')[1]  #mp.group()*/
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('ssspFile',help='path to sssp filenames',action='store')
    parser.add_argument('gamesFile',help='path to games filenames',action='store')

    args = parser.parse_args()
    
    df = pd.read_csv(args.gamesFile, sep=',') 
    df.columns= ['src_pg','game','end_pg'] 
    print df.head()         
    
    spScore = find_sssp_score(df.end_pg, args.ssspFile) 
    #print spScore
