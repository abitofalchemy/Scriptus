...
# -*--*--*--*--*--*--*--*-
# shortestPathsWgameMmap.py 
#	read game data files and find the sssp score to the file's end page_id
#	output a new version of the games data files that includes a column of shortest paths
...
def find_sssp_score(dest_page_id_lst,sssp_file):
    ...
    """
    #print "find_sssp_score", sssp_file
    spScore = []
    with open(sssp_file, 'r') as f:
        with contextlib.closing(mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ)) as m:
            for endPageId in dest_page_id_lst:
                pattern = re.compile(r'^%s\s[0-9]+\s' % endPageId,
                     re.DOTALL | re.IGNORECASE | re.MULTILINE)
                for match in  pattern.findall(m):
                    if match is not None:
                        spScore.append(match[1].replace('\n', ' '))
                if match is None:
    ...
    for endPageId in dest_page_id_lst:
        ...
        for index, row in df.iterrows():
            if row.dest == endPageId:
                ...
                break 
    """
    print 'array sizes', len(spScore), len(dest_page_id_lst)
    return spScore
...
if __name__ == "__main__":
	...
	## Get list of gamesData files 
	gamesFilienames = getFilenames(args.gamesPath)

	for spFile in spFilenames:
		result = re.search('sssp_(.*).txt', spFile)
		#print 'corresponding game data file:', args.gamesPath+result.group(1)+'_wpgame_games.dat'
		...
		if not os.path.exists(outGamesSpFn):
                        print 'Processing: ', gameDataFile, spFile
			df = pd.read_csv(gameDataFile, sep=',') 
			df.columns= ['src_pg','game','end_pg'] 
		        print len(df)
                        f = open(outGamesSpFn,'wb') 
			## Get the list of shortest path scores
			spScore = find_sssp_score(df.end_pg, args.ssspPath+spFile) 
