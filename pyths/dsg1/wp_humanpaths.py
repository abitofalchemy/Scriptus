...
if __name__ == "__main__":
    ...
    query7= "select game_uuid, userid, count(clicked_page) \
         from game_click, \
         (   select clicks.uuid as gcuuid, clicks.userid as cguserid \
            from ( \
            select uuid, game_game.end_page \
            from game_game \
            JOIN wikipedia W ON W.page_title=game_game.start_page \
    ...
    #execQuery(query0B)
    #execQuery(query6,0)
    execQuery(query7,0)  ## query where we use the page_id rather the page_title 
