#!/usr/bin/python
# -*- coding: utf-8 -*-
"""tc_ft_avgg_vis_1.py: Computes the average FFT for a number of records
    trying to get, generates a plot in the images folder """
"""              usage: tc_ft_avgg_vis_1 hostname email or id %""" 
...
#       acMtrx[range(0,len(ta)),i] 
        ...
        print '(min %ld,max%ld)'% (np.min(timeVector),np.max(timeVector)) 
        if (len(avgdAxis) == len(timeVector) ):
            fs = len(timeVector)/(np.max(timeVector)/1e+3)
            if (fs>100):
                print '{Error} sampling frequence exceeds 100 Hz'
                #break 
            else:
                fs_vector[i] = fs
        else:
...
#    print 'shape of x mag spec ', np.shape(mag_spectrum_matrix)
    ...
    #print 'avg fs: ', np.mean(fs_vector,axis=0)
    cur = mydb.cursor()
...
#    print n,', freq:', len(frq)
    ax1.plot(timeVector, avgdAxis)
    ax1.set_ylabel('Acceleration ($m/s^2$)')
    ax1.set_xlabel("Time (ms)")
    x1,x2,y1,y2 = ax1.axis()
    ax1.add_artist(Line2D((x1, x2), (y1, y1), color='black', linewidth=1))
    #print np.min(frq),',',np.max(frq)
    ax2.plot(frq, mean_spec_avg_vec[0:n/2]);
