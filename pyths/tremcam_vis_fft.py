...
def main():
    # input data
    if (len(sys.argv) < 2):
        print "Usage: python tremcam_vis_fft.py hostname user_email" #`"sql query`""
        print "     : or ./tremcam_vis_fft.py hostname user_email"
        exit(0)
    else:
        host = sys.argv[1]
        user = sys.argv[2]
...
#ax1.axhline(0, color='black', lw=1)
    ...
    ax2.grid(True)
    ax2.set_title('Y axis: All',fontsize=8)
    #ax1.axhline(0, color='black', lw=1)
    ax3 = fig.add_subplot(333)
    ax3.grid(True)
    #plt.suptitle(query)
    ax3.set_title('Z axis: All',fontsize=8)
    #ax1.axhline(0, color='black', lw=1)
    ...
    ax5.grid(True)
                #ax5.set_title('Spectrum',fontsize=10)
...
#    #ax1.axhline(0, color='black', lw=1)
    ...
    for i in range(0,N):
        ...
        xa  = [np.double(fv) for fv in xv[i].split(",")]
        avgX        = np.resize(avgX, (len(xa),N))
        avgX[:,i]   = [float(fv) for fv in xa]
        
        t = [float(fv) for fv in tv[i].split(",")]
        tt = [float(fv-t[0]) for fv in t]
        n = range(0,len(xa))
        #ax1.plot(n,tt)
        ax1.plot(tt, xa)
        ya  = [np.double(fv) for fv in yv[i].split(",")]
        n = range(0,len(ya))
        ax2.plot(tt, ya)
        avgY        = np.resize(avgY, (len(ya),N))
        avgY[:,i]   = [float(fv) for fv in ya]
        
        za = [np.double(fv) for fv in zv[i].split(",")]
        n = range(0,len(za))
...
#    ax3.xaxis.set_major_formatter(majorFormatter)
    ...
    ax4.set_ylabel('Averaged (filtered) \n Acceleration', fontsize=8)
    ax5.set_xlabel('Samples', fontsize=8)
#    ax4.set_title('Records averaged followed by a 5-Point Moving-Average Smoothing',fontsize=10)
    ...
    # fft
    mags = abs(fft(smoothedX[0:fftSamples/2]*window))
    # convert to dB
    ...
    ax5.plot(mags,label='X-axis')
    mags = abs(fft(smoothedY[0:fftSamples/2]*window))
    # convert to dB
    mags = 20 * np.log10(mags)
    mags -= max(mags)

    ax5.plot(mags,label='Y-axis')
    mags = abs(fft(smoothedZ[0:fftSamples/2]*window))
    # convert to dB
    mags = 20 * np.log10(mags)
    mags -= max(mags)

    ax5.plot(mags,label='Z-axis')
    ax5.set_ylabel('Normalized\n Magnitude Spectrum)', fontsize=8)
