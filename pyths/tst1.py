...
def compute_plot_fft(arrT, arrDat, nsmp, fs, pHndlr):
    # 'data' consists of x = time vector
    #                    y = data vector
    window  = hann(nsmp)
    print np.shape(arrDat)
    mag_spectrum_y = np.zeros([nsmp,len(arrDat)/nsmp])
    output_vec = np.zeros(nsmp)
#    avgXaxis = np.mean(data[:,0],axis=1)
#    avgYaxis = np.mean(data[:,0],axis=1)
    
    # average 
    for k in range(0,len(data[:,1])/nsmp):
        sliceY = arrDat[k*nsmp:nsmp*(k+1)]
        print k,",",np.shape(sliceY)
        pHndlr.plot(sliceY)
        #multiply it with the window and transform it into frequency domain
        #spectrumx = fft(sliceX*window);
#        spectrumy = fft(sliceY*[w for w in window]);
#        print k,",",np.shape(window)
...
def main():
    ...
    for ix in range(0, len(sys.argv) -2 ):
        ...
        if (ix == 0):
            ...
            compute_plot_fft(timeV[0:K], avgY[0:K,0], 512, fs, ax4)
